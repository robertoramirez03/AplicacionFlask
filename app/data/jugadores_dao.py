...
class Jugadores_dao:
    ...
    def select_all(self,db) -> list[Jugador]:
        cursor = db.cursor()
        cursor.execute('SELECT Jugadores.id, Nombre_jugador, Nombre_Superviviente, Nombre_Asesino FROM rober.Jugadores INNER JOIN rober.Supervivientes on id_superviviente_favorito = Supervivientes.id INNER JOIN rober.Asesinos on id_asesino_favorito = Asesinos.id; ')
